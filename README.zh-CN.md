...
Claude-to-IM 是一个与宿主应用解耦的桥接库，用于将 [Claude Code SDK](https://docs.anthropic.com/en/docs/claude-code/sdk) 连接到飞书/Lark，让用户可以在飞书聊天中直接与 Claude 交互。
...
- **飞书/Lark 适配器**：基于长连接 WSClient 的飞书机器人集成，支持富文本和卡片
- **流式预览**：通过飞书卡片实时展示响应草稿
- **权限管理**：通过交互式内联按钮实现 Claude Code 工具审批（允许 / 拒绝 / 本次会话允许）
- **会话绑定**：每个 IM 聊天映射到一个持久化的会话，支持工作目录和模型配置
- **Markdown 渲染**：将 Claude 响应渲染为飞书富文本/卡片
- **可靠投递**：按飞书限制自动分块、指数退避重试、消息去重
...
| `bridge_feishu_app_id` | 飞书/Lark App ID |
| `bridge_feishu_app_secret` | 飞书/Lark App Secret |
| `bridge_feishu_allowed_users` | 逗号分隔的授权用户 ID |
...
| `bridge_feishu_enabled` | 飞书适配器开关 | `"false"` |
| `bridge_feishu_stream_enabled` | 启用流式预览 | `"true"` |
| `bridge_default_cwd` | 新会话的默认工作目录 | `$HOME` |
| `bridge_model` | 默认 Claude 模型 | 由宿主决定 |

当前运行入口只注册飞书/Lark 适配器。
...
你仍然需要在[飞书开发者后台](https://open.feishu.cn/app)创建自建应用，启用机器人能力，通过长连接订阅 `im.message.receive_v1`，授予 IM 权限，并发布应用。
